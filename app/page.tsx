...
// Using Jikan/MyAnimeList image proxy for real anime art
const ANIME_POSTERS: Record<string, { posters: string[]; color: string; accent: string }> = {
  ...
  "bleach": {
    posters: [
      ...
      "https://cdn.myanimelist.net/images/anime/1764/114529.jpg",
    ],
    color: "#00d4ff", accent: "#7dd3fc",
  },
  "attack on titan": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/10/47347.jpg",
      "https://cdn.myanimelist.net/images/anime/1629/110959.jpg",
    ],
  ...
  "death note": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/9/9453.jpg",
      "https://cdn.myanimelist.net/images/anime/2/246.jpg",
    ],
    color: "#1e1b4b", accent: "#a5b4fc",
  },
  "fullmetal alchemist": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1223/96541.jpg",
    ],
    color: "#92400e", accent: "#fcd34d",
  },
  "dragon ball": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1277/142390.jpg",
    ],
  ...
  "jujutsu kaisen": {
    ...
    color: "#4c1d95", accent: "#c4b5fd",
  },
  "chainsaw man": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1806/126216.jpg",
    ],
    color: "#7f1d1d", accent: "#fca5a5",
  },
  "hunter x hunter": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1337/99013.jpg",
    ],
    color: "#064e3b", accent: "#6ee7b7",
  },
  "vinland saga": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1170/124312.jpg",
    ],
    color: "#1e3a5f", accent: "#93c5fd",
  },
  "berserk": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1384/119988.jpg",
    ],
    color: "#1c1917", accent: "#d6d3d1",
  },
  "vagabond": {
    posters: [
      "https://cdn.myanimelist.net/images/manga/2/192959.jpg",
    ],
    color: "#451a03", accent: "#fed7aa",
  },
  "tokyo ghoul": {
    posters: [
  ...
  "sword art online": {
    ...
    color: "#1e3a5f", accent: "#7dd3fc",
  },
  "solo leveling": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1258/138004.jpg",
    ],
    color: "#1e1b4b", accent: "#818cf8",
  },
  "reverend insanity": {
    posters: [
      "https://cdn.myanimelist.net/images/manga/3/222995.jpg",
    ],
  ...
  "my hero academia": {
    ...
    color: "#1d4ed8", accent: "#93c5fd",
  },
  "spy x family": {
    posters: [
      "https://cdn.myanimelist.net/images/anime/1441/122795.jpg",
    ],
    color: "#be185d", accent: "#f9a8d4",
  },
};

// Find best matching poster for a title
...
// Default themes when no poster match
const DEFAULT_THEMES = [
  { color: "#6b21a8", accent: "#f0abfc", bg: "linear-gradient(135deg, #1a0533, #6b21a8, #ec4899)", emoji: "⚔️" },
  { color: "#1e40af", accent: "#7dd3fc", bg: "linear-gradient(135deg, #0c1445, #1e40af, #06b6d4)", emoji: "🌊" },
  { color: "#92400e", accent: "#fed7aa", bg: "linear-gradient(135deg, #1a0a00, #92400e, #f97316)", emoji: "🔥" },
  { color: "#166534", accent: "#bbf7d0", bg: "linear-gradient(135deg, #0a1a00, #166534, #84cc16)", emoji: "🌸" },
  { color: "#9d174d", accent: "#fda4af", bg: "linear-gradient(135deg, #1a0015, #9d174d, #f43f5e)", emoji: "💀" },
  { color: "#4c1d95", accent: "#c4b5fd", bg: "linear-gradient(135deg, #0f0a1a, #4c1d95, #7c3aed)", emoji: "🔮" },
  { color: "#78350f", accent: "#fde68a", bg: "linear-gradient(135deg, #1a1000, #78350f, #fbbf24)", emoji: "🗡️" },
  { color: "#7f1d1d", accent: "#fca5a5", bg: "linear-gradient(135deg, #1a0a0a, #7f1d1d, #dc2626)", emoji: "🎭" },
];
...
// ─── SLIDING POSTER BANNER ────────────────────────────────────────────────────
function SlidingPosterBanner({ posters, color, accent }: { posters: string[]; color: string; accent: string }) {
  const [idx, setIdx] = useState(0);
  useEffect(() => {
    if (posters.length <= 1) return;
    const t = setInterval(() => setIdx(i => (i + 1) % posters.length), 3000);
  ...
  return (
    <div style={{ position: "relative", height: 140, overflow: "hidden", borderRadius: "16px 16px 0 0" }}>
      ...
      {/* Gradient overlay */}
      <div style={{ position: "absolute", inset: 0, background: `linear-gradient(to bottom, ${color}44 0%, rgba(10,14,26,0.85) 100%)` }} />
      {/* Slide dots */}
      {posters.length > 1 && (
        <div style={{ position: "absolute", bottom: 8, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 4 }}>
          {posters.map((_, i) => (
            <div key={i} onClick={() => setIdx(i)} style={{ width: i === idx ? 16 : 6, height: 6, borderRadius: 3, background: i === idx ? accent : "rgba(255,255,255,0.3)", cursor: "pointer", transition: "all 0.3s" }} />
          ))}
        </div>
      )}
    </div>
      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        <GlassCard hover={false} style={{ padding: 28 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <button onClick={() => setViewDate(new Date(year, month - 1, 1))} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#94a3b8", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontFamily: "inherit", fontSize: 16 }}>‹</button>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
            {days.map((day, i) => {
              ...
              return (
                <div key={i} style={{ aspectRatio: "1", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", fontSize: 12, fontWeight: isToday ? 800 : 500, background: isActive ? "linear-gradient(135deg, #ffd700, #f97316)" : isToday ? "rgba(0,212,255,0.15)" : "rgba(255,255,255,0.03)", border: isToday ? "2px solid #00d4ff" : isActive ? "none" : "1px solid rgba(255,255,255,0.05)", color: isActive ? "#000" : isToday ? "#00d4ff" : isPast ? "#334155" : "#94a3b8" }}>
                  {day}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: qualified ? "#22c55e" : "#ffd700" }}>
            {qualified ? `Streak active! ${streak} day${streak !== 1 ? "s" : ""}` : `Add ${needed} more entr${needed === 1 ? "y" : "ies"} to activate streak!`}
          </span>
        </div>
...
function AuthScreen({ onLogin }: { onLogin: (u: any) => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      } else { user = await signIn(email, password); }
      onLogin(user);
    } catch (e: any) {
      setError(e.code === "auth/user-not-found" ? "No account found." : e.code === "auth/wrong-password" ? "Incorrect password." : e.code === "auth/email-already-in-use" ? "Email already in use." : e.code === "auth/weak-password" ? "Password must be 6+ characters." : "Something went wrong.");
    }
    setLoading(false);
  };
  return (
...
function Leaderboard() {
  ...
  const MEDALS = ["🥇","🥈","🥉"]; const rankColors = ["#ffd700","#c0c0c0","#cd7f32"];
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 60, color: "#475569" }}>Loading...</div> : leaders.length === 0 ? (
        <div style={{ textAlign: "center", padding: 60 }}><div style={{ fontSize: 48, marginBottom: 12 }}>🏆</div><div style={{ fontSize: 16, color: "#64748b" }}>No one yet. Add 3+ entries to appear!</div></div>
      ) : (
          ...
          {leaders.map((leader: any) => (
            <GlassCard key={leader.id} style={{ padding: 16, border: leader.rank<=3?`1px solid ${rankColors[leader.rank-1]}44`:undefined }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ fontSize: leader.rank<=3?28:14, minWidth: 40, textAlign: "center", color: leader.rank<=3?rankColors[leader.rank-1]:"#475569", fontWeight: 800, fontFamily: "'Bebas Neue', cursive" }}>{leader.rank<=3?MEDALS[leader.rank-1]:`#${leader.rank}`}</div>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg, #00d4ff, #ff2254)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, flexShrink: 0 }}>{(leader.displayName||"?")[0].toUpperCase()}</div>
                <div style={{ flex: 1 }}>
...
function ErrorBook({ userId, onUpdate }: { userId: string; onUpdate: () => void }) {
  const [errors, setErrors] = useState<any[]>([]); const [showForm, setShowForm] = useState(false);
  const [filterSubject, setFilterSubject] = useState("All"); const [filterMistake, setFilterMistake] = useState("All");
...
function AnimeCollection({ userId, onUpdate }: { userId: string; onUpdate: () => void }) {
  const [collection, setCollection] = useState<any[]>([]); const [filterType, setFilterType] = useState("All"); const [filterStatus, setFilterStatus] = useState("All"); const [loading, setLoading] = useState(true);
  useEffect(() => { getCollection(userId).then(d => { setCollection(d); setLoading(false); }); }, [userId]);
  const handleAdd = async () => {
    const title = prompt("Enter anime/manga title:");
    if (!title) return;
  ...
  const filtered = collection.filter(a => { if (filterType!=="All"&&a.type!==filterType) return false; if (filterStatus!=="All"&&a.status!==filterStatus) return false; return true; });
  const chipStyle = (active: boolean, color: string) => ({ padding: "5px 12px", borderRadius: 20, border: `1px solid ${active?color:"rgba(255,255,255,0.1)"}`, background: active?`${color}22`:"transparent", color: active?color:"#64748b", fontSize: 12, cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s" });
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#475569" }}>Loading...</div> : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 14 }}>
          {filtered.length===0 && <div style={{ textAlign: "center", padding: 40, color: "#475569", gridColumn: "1/-1" }}>No entries yet! Type an anime name to add 🎌</div>}
          {filtered.map((item, idx) => {
            const match = findAnimePoster(item.title);
            const fallback = DEFAULT_THEMES[idx % DEFAULT_THEMES.length];
            const accent = match ? match.accent : fallback.accent;
            return (
              <GlassCard key={item.id} style={{ padding: 0, overflow: "hidden", border: "none" }}>
                {/* Smart poster banner */}
                {match ? (
                  <SlidingPosterBanner posters={match.posters} color={match.color} accent={match.accent} />
                ) : (
                  <div style={{ height: 140, background: fallback.bg, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" as const, overflow: "hidden", borderRadius: "16px 16px 0 0" }}>
                    ...
                    <div style={{ position: "absolute", top: -20, right: -20, width: 80, height: 80, borderRadius: "50%", background: `radial-gradient(circle, ${accent}44, transparent)` }} />
                    <span style={{ fontSize: 52, filter: `drop-shadow(0 0 16px ${accent})`, position: "relative", zIndex: 1 }}>{fallback.emoji}</span>
                  </div>
                )}
                {/* Status badge overlaid */}
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", top: -130, right: 10, padding: "3px 10px", borderRadius: 12, fontSize: 10, fontWeight: 700, background: `${STATUS_COLORS[item.status]}33`, color: STATUS_COLORS[item.status], border: `1px solid ${STATUS_COLORS[item.status]}66`, backdropFilter: "blur(4px)" }}>{item.status}</span>
                </div>
                <div style={{ padding: 16, background: "rgba(10,14,26,0.97)" }}>
                  ...
                  {match && <div style={{ fontSize: 10, color: accent, marginBottom: 6, letterSpacing: 1 }}>✦ MATCHED POSTER</div>}
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const, marginBottom: 10 }}>
                    {(item.tags||[]).map((tag: string)=>(<span key={tag} style={{ padding: "2px 8px", borderRadius: 12, fontSize: 10, background: `${accent}18`, color: accent, border: `1px solid ${accent}33` }}>{tag}</span>))}
                  </div>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ fontSize: 10, color: "#64748b" }}>⚡ POWER LEVEL</span><span style={{ fontSize: 11, fontWeight: 700, color: accent }}>{(item.powerLevel||0).toLocaleString()}</span></div>
                    <div style={{ height: 4, background: "rgba(255,255,255,0.05)", borderRadius: 2, overflow: "hidden" }}><div style={{ height: "100%", width: `${((item.powerLevel||0)/9999)*100}%`, background: `linear-gradient(90deg, ${accent}, #ff6b35)`, borderRadius: 2 }} /></div>
                  </div>
                  {item.notes && <div style={{ fontSize: 11, color: "#64748b", borderLeft: `2px solid ${accent}44`, paddingLeft: 8, marginBottom: 10 }}>{item.notes}</div>}
...
export default function App() {
  const [user, setUser] = useState<any>(null); const [authLoading, setAuthLoading] = useState(true);
  const [activeTab, setActiveTab] = useState("errors"); const [quoteIdx, setQuoteIdx] = useState(0);
  const [streak, setStreak] = useState(0); const [todayCount, setTodayCount] = useState(0);
  ...
  if (authLoading) return (
    <div style={{ minHeight: "100vh", background: "#050810", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ fontSize: 48 }}>⚡</div></div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050810", color: "#e2e8f0", fontFamily: "'DM Sans', sans-serif", position: "relative" }}>
      ...
      <Particles />
      {showCalendar && <StreakCalendar userId={user.uid} streak={streak} onClose={()=>setShowCalendar(false)} />}

...
function ErrorBook({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    getErrors(userId).then((data) => { setErrors(data); setLoading(false); });
  }, [userId]);

  const handleAdd = async (form: any) => {
    const docRef = await addError(userId, form);
    setErrors((p) => [{ id: docRef.id, ...form }, ...p]);
  };
  ...
  const thisWeek = errors.filter((e: any) => {
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 40, color: "#475569" }}>Loading your errors...</div>
      ) : (
...
function AnimeCollection({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    getCollection(userId).then((data) => { setCollection(data); setLoading(false); });
  }, [userId]);
  ...
  const handleAdd = async () => {
    ...
    const docRef = await addCollectionEntry(userId, entry);
    setCollection((p) => [{ id: docRef.id, ...entry }, ...p]);
  };

  const handleDelete = async (id: string) => {
    await deleteCollectionEntry(id);
    setCollection((p) => p.filter((e) => e.id !== id));
  };
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 40, color: "#475569" }}>Loading your collection...</div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 14 }}>
          {filtered.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "#475569", gridColumn: "1/-1" }}>No entries yet. Add your first! 🎌</div>}
            ...
            <GlassCard key={item.id} style={{ padding: 0, overflow: "hidden" }}>
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const, marginBottom: 10 }}>
                  {(item.tags || []).map((tag: string) => (<span key={tag} style={{ padding: "2px 8px", borderRadius: 12, fontSize: 10, background: "rgba(168,85,247,0.15)", color: "#a855f7", border: "1px solid rgba(168,85,247,0.3)" }}>{tag}</span>))}
                </div>
                ...
                <div style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ fontSize: 10, color: "#64748b" }}>⚡ POWER LEVEL</span><span style={{ fontSize: 11, fontWeight: 700, color: "#ffd700" }}>{(item.powerLevel || 0).toLocaleString()}</span></div>
                  <div style={{ height: 4, background: "rgba(255,255,255,0.05)", borderRadius: 2, overflow: "hidden" }}><div style={{ height: "100%", width: `${((item.powerLevel||0)/9999)*100}%`, background: "linear-gradient(90deg, #ffd700, #ff6b35)", borderRadius: 2 }} /></div>
                </div>
                {item.notes && <div style={{ fontSize: 11, color: "#64748b", borderLeft: "2px solid rgba(255,255,255,0.1)", paddingLeft: 8, marginBottom: 10 }}>{item.notes}</div>}
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuth(async (u) => {
      ...
      if (u) {
        await recordLoginDate(u.uid);
        const s = await getStreak(u.uid);
        setStreak(s);
      }
    });
    return () => unsub();
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050810", color: "#e2e8f0", fontFamily: "'DM Sans', sans-serif", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1100, margin: "0 auto", padding: "0 16px" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 20, background: "rgba(255,215,0,0.1)", border: "1px solid rgba(255,215,0,0.2)" }}>
              <span style={{ fontSize: 14 }}>🔥</span><span style={{ fontSize: 12, color: "#ffd700", fontWeight: 700 }}>{streak} day streak</span>
            </div>
        ...
        {activeTab === "errors" && <ErrorBook userId={user.uid} />}
        {activeTab === "collection" && <AnimeCollection userId={user.uid} />}
      </div>
    </div>
  );
}